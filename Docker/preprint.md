...
- [Всё, что нужно знать о Docker](#всё-что-нужно-знать-о-docker)
  - [Оглавление:](#оглавление)
- [Docker с 0 до 100%. Всё, что нужно знать.](#docker-с-0-до-100-всё-что-нужно-знать)
  - [Установка Docker скриптом](#установка-docker-скриптом)
  - [Начальная настройка Docker (Ubuntu)](#начальная-настройка-docker-ubuntu)
  - [Основные команды Docker](#основные-команды-docker)
  - [Port mapping – проброс портов контейнера](#port-mapping--проброс-портов-контейнера)
  - [Environment variables - Переменные среды](#environment-variables---переменные-среды)
  ...
  - [Docker Networks - Сетевые настройки](#docker-networks---сетевые-настройки)
    - [Ключевые различия режимов:](#ключевые-различия-режимов)
    - [Несколько интерфейсов:](#несколько-интерфейсов)
      - [Примеры использования нескольких сетевых интерфейсов в Docker-контейнере:](#примеры-использования-нескольких-сетевых-интерфейсов-в-docker-контейнере)
  - [Dockerfile - Файл докера](#dockerfile---файл-докера)
  - [Docker Compose. Докер Компос](#docker-compose-докер-компос)
    - [Установка Docker Compose в Ubuntu:](#установка-docker-compose-в-ubuntu)
    - [Стратегии поведения контейнера:](#стратегии-поведения-контейнера)
    - [Пример управления несколькими контейнерами:](#пример-управления-несколькими-контейнерами)
  - [Практичные примеры:](#практичные-примеры)
    - [Пример запуска Nextcloud:](#пример-запуска-nextcloud)
    - [Пример запуска Wordpress:](#пример-запуска-wordpress)
  - [Очистка и сброс Docker Compose](#очистка-и-сброс-docker-compose)
    - [Шаг 1. Останов и удаление контейнеров:](#шаг-1-останов-и-удаление-контейнеров)
    - [Шаг 2. Удаление  сетей:](#шаг-2-удаление--сетей)
    - [Шаг 3. Удаление томов Docker:](#шаг-3-удаление-томов-docker)
    - [Шаг 4. Удаление образов контейнеров:](#шаг-4-удаление-образов-контейнеров)
    - [Шаг 5. Очистка системы:](#шаг-5-очистка-системы)
    - [Шаг 6. Удаление проекта Compose:](#шаг-6-удаление-проекта-compose)
  - [Portainer. Управление Docker в графическом режиме](#portainer-управление-docker-в-графическом-режиме)
    - [Настройка Portainer:](#настройка-portainer)
    - [Пример создания контейнера Nginx из шаблона:](#пример-создания-контейнера-nginx-из-шаблона)
    - [Пример деплоя блог - платформы Ghost посредством Стека:](#пример-деплоя-блог---платформы-ghost-посредством-стека)
    - [Подключение других серверов с Docker:](#подключение-других-серверов-с-docker)
    - [Пример подключения сервера через Portainer Agent:](#пример-подключения-сервера-через-portainer-agent)
...
> 
> Команда запустит контейнер работающий в фоновом режиме `-d` 15 минут `sleep 900`, с тегом `20.04`, c именем `--name` “Ubuntu20” . После остановки контейнер будет удален `--rm`.
...
> `/bin/bash` - указывает какой терминал будет запущен внутри контейнера, некоторые контейнеры не имеют `bash`, тогда нужно узнать на сайте разработчика, какой терминал присутствует в контейнере, например часто бывает `/bin/sh`

Полностью все удалить (очистка всех неиспользуемых ресурсов - остановленные контейнеры и образы)
```bash
docker system prune -a --volumes
```
>> **Осторожно!** Параметр `--volumes` удаляет тома. Команда в данном виде, при условии что все контейнеры остановлены - удаляет всё наверняка!
...
>> **Будьте осторожны, при использовании команды `prune`!**
...
Эта команда состоит из двух: нахождение пути к файлу журнала — ```docker container inspect  --format='{{.LogPath}}' <container_name_or_id>```, в результате будет получен путь к файлу журнала. Сокращение файла журнала: ```truncate -s 0 /path/to/logfile```

> `-s` используется для установки размера файла. При нулевом значении в качестве входных данных, происходит полное усечение файла журнала.Таким образом можно не полностью очищать журнал контейнера, а сокращать его до заданного размера.
...
При использовании параметра `-P` (`--publish-all`) — автоматически будут назначены порты со стороны сервера, со стороны контейнера будут назначены порты, прописанные в директиве `EXPOSE` в `dockerfile` (См. ниже [dockerfile](#dockerfile---файл-докера)).
```bash
docker run -P <имя образа>
```

> **ВАЖНО:** `-P` Не рекомендуется использовать в продакшене. Для большего контроля портов, в проде - рекомендуется параметр `-p <порт_сервера>:<порт_контейнера>`.
...
По другому - ***Persisting Data** - Сохраняющиеся данные*. Docker имеет три способа сохранять данные.

**Host volumes:** 

Пространства на физическом хосте. Позволяет монтировать директории физического сервера, хоста — в выбранную директорию в контейнере
```bash
...
Анонимное пространство — в команде не указывается путь к пространству на физическом сервере. Docker создает его (анонимное пространство) сам. 
...
Именные пространства монтируются на физическом хосте там же, где и анонимные пространства, только с именем. В команде указывается произвольное имя, по которому будет называться его смонтированный на хосте каталог.
...
```bash
docker network disconnect <ID сети> <имя контейнера>  # ID сети можно посмотреть с помощью команда inspect (см выше).
```
...
CMD [“start.sh”] 	# Команда после этой инструкции является как-бы по умолчанию, и ее можно переписать указанием команды при запуске контейнера.
```
...
```bash
docker-compose -f my_custom_name.yml up
```

Можно использовать это другое имя файла по умолчанию (чтобы каждый раз не указывать `-f` с именем файла), для этого нужно изменить переменную окружения `COMPOSE_FILE`. Чтобы использовать файл с именем `my_docker_compose.yml` по умолчанию, нужно выполнить:
```bash
export COMPOSE_FILE=my_docker_compose.yml
```
...
- базы данных на postgress `(app-db)` 
...
```
В `docker ps -aq` перечислены все идентификаторы контейнеров, которые передаются в `docker rm` для удаления.
> **ВАЖНО:** при этом удаляются не только контейнеры **compose**, но и все остальные *остановленные* контейнеры.
...
> **Заметка:** при создании тома, можно *ограничить* его по размеру. Это следует делать разумно, т.к. тома содержат ценные данные.
...
Для выборочного удаления образов можно использовать удаление по `тегу` или `ID`
```bash
...
> **Заметка:** можно использовать политики переназначения тэгов образов, чтобы избежать устаревших артефактов и мониторинг для обнаружения быстрого роста заполнения  дисков.
...
Резюмируя, можно кратко еще раз обозначить: 

**Основные правила очистки и сброса Docker Compose**:
...
> Данная ссылка ведет на установку версии **Portainer Community Edition**, для установки  **Business Edition** - нужно убрать `-ce` из ссылки.
...
- Интерес представляют шаблоны (`templates`) стандартных базовых контейнеров, находящиеся в `local/App Templates`. 
  ...
  - вэб движки и т.д. 
  - Так-же имеются *стеки* контейнеров, например `WordPress` c `MySQL`.
...
- Далее можно в браузере зайти на настроенный ранее (редактирование портов) адрес. 
...
- Далее нужно проверить работу *агента* путем вызова списка запущенных контейнеров — `docker ps`. *Portainer_agent* — должен быть запущен.
...
Одно из преимуществ *Portainer* состоит в том, что устанавливать и использовать все его удобства и возможности - можно гораздо позже начала существования *Docker инфраструктуры*. 

Другими словами - для всей полноты использования функционала *Portainer*, не нужно его использовать с самого старта инфраструктуры. *Portainer* можно подключить в любой момент ее существования, без каких-либо ограничений, которые могли бы возникнуть. 

*Portainer* управляет уже созданными до его появления контейнерами без каких-либо ограничений.

В общем - в *Portainer’e*, нужно понять всего одно его условие:  *для тех кто знает командную строку докера - разобраться в Portainer’e будет не сложно.* 

**Но не наоборот!** 

Поэтому если вдруг, дорогой читатель решил кушать пиццу с середины - то см.выше с самого начала. В любом знании важен фундамент. :) 
