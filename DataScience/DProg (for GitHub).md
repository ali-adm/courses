...
### [Техники запросов](#query_techniq)
...
1. Какая команда SQL используется для сортировки извлеченных данных?

- [x] ORDER BY
- [ ] WHERE
- [ ] SORT

2. По умолчанию команда ORDER BY сортирует данные в ...

- [x] возрастающем порядке
- [ ] убывающем порядке
- [ ] случайном порядке

3. Какое ключевое слово используется для сортировки данных от большего к меньшему или от Z до A?

- [x] DESC
- [ ] ASC
- [ ] DOWN

4. Какова цель ключевого слова LIMIT?

- [ ] Конкатенация строк
- [x] Сокращение количества извлеченных записей
- [ ] Сортировка данных

5. Какая правильная последовательность при использовании ORDER BY и LIMIT вместе?

- [x] Сначала сортировка, затем ограничение
- [ ] Сначала ограничение, затем сортировка

6. Какое ключевое слово используется для фильтрации данных на основе определенных условий?

- [ ] IF
- [x] WHERE
- [ ] FILTER

7. Сколько полей будет содержать таблица результатов после выполнения этого запроса?
```sql
SELECT name, price, price + delivery
FROM sales
WHERE id > 3
...
8. Завершите запрос для извлечения 5 самых дорогих товаров.
```sql
SELECT *
FROM products
ORDER BY price DESC
...
9. Завершите запрос для извлечения книг с рейтингом выше 7.
```sql
SELECT *
FROM books
WHERE rating > 7
...
10. Извлеките имена студентов, чья страна - 'USA'
```sql
SELECT name
FROM students
WHERE country = 'USA'
...
11. Извлеките сотрудников, у которых менее 5 лет опыта
```sql
SELECT *
FROM employees
WHERE experience < 5
...
12. Завершите запрос для извлечения имен и электронных адресов пользователей, которые в настоящее время активны
```sql
SELECT name, email
FROM users
WHERE status = 'active'
...
Пример запроса для фильтрации записей студии с именем `'Walt Disney'`
```sql
SELECT *
FROM studios
WHERE name = 'Walt Disney'
...
Или запрос фильтрации записей фильмов с заданным именем `'Frozen'`:
```sql
SELECT *
FROM movies
WHERE title = 'Frozen'
...
![alt text](https://lecontent.sololearn.com/material-images/f7ae3771bfd844b8a834d9cac04ccda7-3.01.05.png)

Сколько комиксов будет извлечено этим запросом?
```sql
SELECT *
FROM comics
...
Следующий код демонстрирует запрос для извлечения всех комиксов **The Avengers**:
```sql
SELECT *
FROM comics
...
Выберите строку, которая соответствует шаблону `'%gmail.com'`
- [ ] r.simpsons@mail.com
- [ ] r.simpsons@gmail.us
- [x] r.simpsons@gmail.com
...
**Подстановочный знак %** может использоваться в любой части шаблона и столько раз, сколько необходимо.

Следующий код извлекает **все** названия, содержащие слово **Avengers**
```sql
SELECT *
FROM comics
...
Какой шаблон следует использовать, чтобы соответствовать только строкам, **начинающимся** с символа `'A'`?
- [ ] '%A'
- [x] 'A%'
- [ ] '%A%'
...
Выберите название продукта, которое будет извлечено
```sql
SELECT name
FROM products
...
Символ `%` означает любое количество символов, включая ноль символов.

Какой шаблон соответствует значениям `'service fee'` и `'fee refund'`?
- [ ] 'fee%'
- [x] '%fee%'
- [ ] '%fee'
...
Когда числа являются частью строки, их также можно сопоставлять с шаблонами.

![alt text](https://lecontent.sololearn.com/material-images/4461dfa0c5854939a6a4b9cc831d7ed8-3.01.16.png)

Например следующим запросом - будет извлечена запись с `id` равным единице:
```sql
SELECT *
FROM comics
...
Какие строки будут соответствовать шаблону `'Package%1%'`?
- [x] Package 10E: Бизнес-план
- [ ] Package 25A: Экономический план
- [x] Package AD1: Промежуточный план

### [Назад к оглавлению](#back)


<a name="pract_chocolad"></a>
## Шоколад! (практика)
...
![alt text](https://lecontent.sololearn.com/material-images/0ad4e9667ec049fabf7504359c7e1625-f129f03a8610428a924ba36c1e2842ce-draftcc11.png)
...
Следуюищий запрос извлечет записи песен, которые были выпущены после 2020 года:
```sql
SELECT *
FROM songs
WHERE year > 2020
...
А этот запрос извлечет песни, выпущенные в 2020 году:
```sql
SELECT *
FROM songs
WHERE year = 2020
...
Операция сравнения всегда дает либо Правда `True`, либо Ложь `False`.
- 15 > 20 - `False`
- 7 < 13 - `True`

Вы уже знакомы с операторами сравнения >, < и =. Существуют и другие операторы сравнения, которые можно использовать для более точных запросов.
- больше или равно `>=`
- меньше или равно `<=`
- не равно `<>`
...
![alt text](https://lecontent.sololearn.com/material-images/37858e877d92409990f8c79cdd5b5b73-3.03.08.png)

Какие фильмы соответствуют условию year `>= 2003`?
- [ ] Jurassic Park
- [x] Pirates of the Caribbean
- [x] Frozen

Какие фильмы соответствуют условию year `<> 2013`?
- [x] Все, кроме Frozen
- [ ] Все

Следуюищий запрос извлечет `Star Wars`:
```sql
SELECT title
FROM movies
...
Следующий запрос не извлечет ничего, т.к. есть ошибка (отсутсвуют одинарные кавычки вокруг строки):
```sql
SELECT title
FROM movies
...
Ниже приведен код, который даст две таблицы. Первая помогает нам увидеть, какие записи соответствуют условию. Вторая - результат фильтрующего запроса.
```sql
/*
...
Следующий запрос извлекает только товары, имеющиеся в наличии:
```sql
SELECT name
FROM products
WHERE quantity <> 0;
...
![alt text](https://lecontent.sololearn.com/material-images/eeab51d4f009455badd26cc041db4c52-3.03.17.png)
...
Т.е. этот запрос извлекает записи песен, которые **не относятся к категории `Jazz`**.

Еще запрос, извлекающий песни, которые не превышают три минуты:
```sql
SELECT titles
FROM songs
WHERE length < 3
...
![alt text](https://lecontent.sololearn.com/material-images/591e3064aa3444e48644009e897ae783-d828b1794b4246418b627be00d7e7cb7-draftcc13.png)

Напишите запрос для выбора `make` машины, `model` и `price per day` из таблицы машин, где цена за день меньше или равна `$100`.
```sql
/*
table name: cars
...
<a name="data_agregation"></a>
## Агрегация данных

- **операции агрегации** суммируют данные, чтобы получить одно единственное числовое значение
...
![alt text](https://lecontent.sololearn.com/material-images/2d4f94c59b474dce9d9e6a096097cf37-3.04.02.png)


**MAX()** - это пример операции агрегации. Какое максимальное значение для поля year?
```sql
MAX(year)
```
- [x] 1993
...
Результатом операции агрегации является одно числовое значение. 

**MIN()** - еще один пример операции агрегации. Каков результат этой операции?
```sql
MIN(year)
```
- [ ] 1990
...
**Агрегация** работает с **несколькими значениями**, для получения **одного единственного значения**.
...
Следующий запрос извлекает самый старый фильм:
```sql
SELECT MIN(year)
FROM movies;
```

Следующий запрос генерирует одно числовое значение потому что - **операции агрегации производят одно число**:
```sql
SELECT MAX(year)
FROM movies
```
...
**COUNT()** - это еще одна операция агрегации. Она подсчитывает количество записей:
```sql
SELECT COUNT()
FROM movies;
```

![alt text](https://lecontent.sololearn.com/material-images/dd9819504e3a45c29054f2504ead9335-3.04.02.png)
...
Какое числовое значение запрос произведет на основе таблицы выше?
- [ ] 1
- [x] 3

Сколько числовых значений произведет запрос?
- [ ] 9
- [x] 1
...
![alt text](https://lecontent.sololearn.com/material-images/269f48070ab04b11b7fc3e7230ce213d-3.04.14.png)
...
**AVG()** - это операция агрегации. Это означает, что запрос приведет к одному единственному числовому значению.

### [Назад к оглавлению](#back)


<a name="data_mixing"></a>
...
Этот запрос выше приводит к одному числовому значению.

Ниже - запрос для вычисления самой высокой цены:
```sql
SELECT MAX(price)
FROM products;
```

![alt text](https://lecontent.sololearn.com/material-images/a4c9559a0cf64e1395f5861a9e28a4e1-2.05.05.png)
...
Запрос выше извлечет две записи о фильмах. Вы можете выбирать выполнение агрегационных операций только для записей, удовлетворяющих определенному условию. Ниже пример запроса, который объединяет фильтрацию с агрегационной операцией:
```sql
SELECT MIN(year)
FROM movies
WHERE year > 2000
...
Запрос, который подсчитывает количество записей после фильтрации:
```sql
SELECT COUNT(price)
FROM products
WHERE price > 10
...
![alt text](https://lecontent.sololearn.com/material-images/a0dba63ec7224050815fa873cb93bbc5-2.03.13.png)
...
При выполнении запросов, включающих различные операции, сначала происходит фильтрация. Результатом запроса выше - будет `2003`.

Если запрос содержит операции агрегации и фильтрации, **операция фильтрации происходит первой**!

Далее предствлен запрос для вычисления суммы цен для продаж, где статус `'completed'`:
```sql
SELECT SUM(price)
FROM sales
...
![alt text](https://lecontent.sololearn.com/material-images/db8d4b1caa8b4177ab4c395257484dde-3.05.11.png)
...
![alt text](https://lecontent.sololearn.com/material-images/758258412ac24cf793d15c8961dc1509-3.05.13.png)
...
Запрос вычислит два числовых значения: `2013` и `2003`.

![alt text](https://lecontent.sololearn.com/material-images/e18a3de097264f8b9d8952599660f65a-3.05.11.png)
...
WHERE status = 'completed'
```
Каким будет результат этого запроса?
- [ ] 26
- [x] 21
...
![alt text](https://lecontent.sololearn.com/material-images/0eb6c804edde44a8a2443ac6277edd26-SQL4.01.01.png)

В этом уроке вы узнаете о первых шагах в процессе **анализа данных**, начиная с **вопросов**.

![alt text](https://lecontent.sololearn.com/material-images/84bec07c795e46c7823e1c8dc07cea50-3.03.08.png)

Выберите вопрос, на который можно ответить с помощью информации, хранящейся в наборе данных о `movies`.
- [x] Какой самый старый фильм?
...
На какой вопрос отвечает следующий запрос?
```sql
SELECT COUNT(title)
FROM movies
```

- [ ] Количество студий в базе данных
- [ ] Самый старый фильм в базе данных
- [x] Количество фильмов в базе данных

![alt text](https://lecontent.sololearn.com/material-images/3215cccd214243ee9ddb57762b4bbe06-1.04.11(2).png)

Схема базы данных помогает нам проверить, можно ли ответить на вопрос с помощью данных. Схема...

- [ ] показывает количество записей в таблице
- [x] визуализирует, как организована база данных

Выберите вопрос, на который можно ответить с помощью информации в этой базе данных.
- [ ] Сколько сотрудников ездят на работу на велосипеде?
- [x] Какой самый большой отдел?

Какая информация включена в схему? 
- [x]  имена полей
- [x]  ключи
- [x]  имена таблиц
- [ ]  записи

Количество записей меняется при добавлении новых данных в базу данных. Схема...

- [x] не показывает количество записей в таблицах
- [ ] показывает количество записей в таблицах

Схема не показывает количество записей в таблицах базы данных, потому что...

- [ ] это число фиксировано
- [x] это число может измениться

![alt text](https://lecontent.sololearn.com/material-images/004240e440cc401fbe1ba2339cf567c8-1.04.05.png)
...
На некоторые вопросы можно ответить с помощью фильтрующего запроса. Завершите фильтрующий запрос, чтобы определить студентов, у которых оценка 100:
```sql
SELECT name
FROM students
WHERE grade = 100;
...
Завершите, чтобы определить пользователей, которые достигли или превысили уровень 10: 
```sql
SELECT username
FROM users
WHERE level >= 10;
...
На какой вопрос отвечает этот запрос?
- [ ] Количество студентов в классе
- [x] Средний балл в классе
- [ ] Самый высокий балл в базе данных

![alt text](https://lecontent.sololearn.com/material-images/c3a2af7102e44937836be54b9c02af3e-Frame27153(1).png)

После того, как ваш вопрос был определен, следующим шагом обычно является **сбор данных** для его ответа.

Данные непрерывно генерируются и могут поступать из разных источников.

Сопоставьте данные с их типом источника:
- даты твитов: **социальные данные**
- пульс: **данные устройства**
- суммы платежей: **транзакционные данные**

![alt text](https://lecontent.sololearn.com/material-images/d38a41d87d4c4075b3c3b394e03b04b7-4.01.16.png)
...
Выберите все методы, которые вы можете использовать для сбора данных:
- [x] Запрос данных из базы данных
- [x] Подключение к серверам с помощью API
- [x] Скрапинг веб-страниц
...
**Группировка данных** - это мощный инструмент при работе с большими наборами данных. Группировка позволяет собирать и просматривать данные по-новому, чтобы ответить на более сложные вопросы. На этом уроке вы узнаете, как группировать информацию, чтобы видеть закономерности и взаимосвязи.

Группировка часто используется в сочетании с агрегацией. Давайте разомнемся! Следуюищий запрос агрегации приведет к одному числовому значению:
```sql
SELECT MAX(price)
FROM products
```

Или, например, запрос, извлекающий средний `rating` для `products`:
```sql
SELECT AVG(rating)
FROM products
```

![alt text](https://lecontent.sololearn.com/material-images/a98e40130a884fe081ec92aaadebb00a-sql4.02.04.png)

**GROUP BY** позволяет организовать похожие данные в категории. Он используется в сочетании с агрегацией для вычисления ключевых показателей для группы записей. Для группировки данных требуется хотя бы одно поле. Данные в этой таблице результатов сгруппированы по **жанру**:
```sql
...
![alt text](https://lecontent.sololearn.com/material-images/454a35f58b48476cb89f9262d2009460-Frame27145.png)

Еще пример SQL-запрос для вычисления суммы бюджетов для разных жанров фильмов:
```sql
...
![alt text](https://lecontent.sololearn.com/material-images/5b119f2abc9b4bfab038c002ccaab73b-SQL4.02.07.png)

Запрос для вычисления общего количества каждого продукта:
```sql
...
Запрос для подсчета продуктов в каждой группе категорий:
```sql
...
![alt text](https://lecontent.sololearn.com/material-images/abe1c385bda845afa0d36e333ec39416-Frame27201.png)
...
На основе таблицы фильмов выше, сколько средних значений будет вычислено этим запросом?
- [x] 3
- [ ] 1

![alt text](https://lecontent.sololearn.com/material-images/8b559cba45ed4fb98a73d302007ce80d-SQL4.02.12.png)

Этот запрос сгенерирует таблицу результатов с…
- [x] 2 категориями и 2 числовыми значениями
- [ ] 3 категориями и 3 числовыми значениями
- [ ] 1 категорией и 2 числовыми значениями

![alt text](https://lecontent.sololearn.com/material-images/936c4d14605940a687b846a28865ca77-SQL4.02.13.png)
...
Можете ли вы исправить код, чтобы включить группу в таблицу результатов?
```sql
...
**GROUP BY** помещает данные с одинаковыми значениями в одну категорию. Данные в одной группе имеют одно и то же значение для...
- [ ] записи (строки)
- [x] поля (столбца)

GROUP BY чаще всего используется в сочетании с...
- [ ] LIMIT
- [ ] OFFSET
- [x] агрегацией

Запрос для вычисления количества для каждой категории:
```sql
...
![alt text](https://lecontent.sololearn.com/material-images/ce0244823b7247839f57d42263dd2d64-190a8cc04f4e4e078f999a649e8bcc6f-grouping2.png)
...
<a name="data_fixing1"></a>
## Исправление типов данных 1

- Значения данных могут классифицироваться по разным **типам данных**
- Тип данных **булев** имеет два возможных значения:  `True` или `False` (Правда или Ложь)
...
![alt text](https://lecontent.sololearn.com/material-images/394d68c1ecf741c4949d078f132a36c1-Fraf(3).png)

Поля в этой таблице содержат разные типы данных.
- description - **строка**
- priority - **числовой**

![alt text](https://lecontent.sololearn.com/material-images/c0a164322f74417586205946f402bd0c-4.05.03.png)

Сопоставьте поле и тип данных:
- name: **строка**
- age: **числовой**
- appointment: **дата/время**

![alt text](https://lecontent.sololearn.com/material-images/e39f69b73ed7442fab67d511d2f785e1-4.05.04.png)

Определите проблемы качества данных
- дублирование: **3**
...
WHERE title = 'Home Alone';
```
Некоторые типы данных явно указаны в SQL-коде. Как вы узнаете, что значение является строкой?

- [x] Значение окружено кавычками
- [ ] Строка начинается с символа решетки `#`
- [ ] Присутствует знак равенства `=`

![alt text](https://lecontent.sololearn.com/material-images/49b3a6928372429598f37c40aa01f097-4.05.07.png)

Определите разные типы данных в таблице:
- [x] Строковый
- [x] Числовой
- [ ] Температура
- [x] Дата/время

Вы собираетесь познакомиться с новым типом данных.

Сначала, какой будет результат этой операции сравнения?
```sql
150 > 250
```
- [x] Ложь (`False`)
...
Электронные схемы внутри компьютеров используют миллионы маленьких переключателей для хранения значений Правда/Ложь.

![alt text](https://lecontent.sololearn.com/material-images/832fb62932bf4f038e10d3c74a179169-Frame27014.png)

Сколько позиций у этих переключателей?
- [ ] 3 (НИЗКО, СРЕДНЕ и ВЫСОКО)
- [x] 2 (ВКЛ и ВЫКЛ)
...
Что означает двоичный в этом контексте?
- [ ] код состоит из двух строк
- [x] два варианта состояния переключател

![alt text](https://lecontent.sololearn.com/material-images/19bf1eaf73e24e6e9139334be919c47f-4.05.11.png)

Тип данных, который хранит одно из двух возможных значений `True` или `False` (Правда или Ложь), называется булевым.

Какое поле в таблице хранит булевы значения?
- [ ] email
- [x] is_verified

![alt text](https://lecontent.sololearn.com/material-images/23e02a960f02438c87c7e73166c48538-4.05.12.png)

Выберите поле с булевыми значениями
- [ ] genre
- [ ] budget
- [x] is_oscar_winner

Выберите названия фильмов, которые получили Оскар:
- [X] Star Wars
- [ ] Home Alone
- [X] Forrest Gump
- [X] Titanic

Вы можете использовать булевы значения при фильтрации данных:
```sql
...
Выберите все булевы значения:
- [ ] 18
- [ ] "Avatar"
- [x] True
- [x] False

![alt text](https://lecontent.sololearn.com/material-images/5355a0fce2924fc6a8ce2c9c690d6665-4.05.16.png)

Результат операции сравнения всегда является булевым значением. Булевы значения используются для фильтрации данных:
```sql
SELECT title
FROM movies
WHERE budget > 50;
...
![alt text](https://lecontent.sololearn.com/material-images/2f09f9bbe4e7474aaaafa0ea4de53119-4.05.17.png)

Выберите идентификаторы фильмов, которые соответствуют условию фильтрации:
- [x] 3
- [x] 4
- [x] 5
- [ ] 1

![alt text](https://lecontent.sololearn.com/material-images/6b843a0e0a4b4e51aa11138c2f614530-4.05.18.png)

Сколько записей будет выбрано этим запросом?
- [ ] 4
- [x] 3
- [ ] 2
...
![alt text](https://lecontent.sololearn.com/material-images/2180640e4df149b495893258f714175c-Frame7155(1).png)
...
![alt text](https://lecontent.sololearn.com/material-images/f8e6f02ccc964a48b59aad34c0309ab3-Frame27184.png)

Запросы данных невероятно полезны для извлечения информации из данных и ответов на вопросы. Ниже пример кода, который позволяет найти самый продаваемый продукт в июне:
```sql
...
Результатом операции сравнения всегда является…
- [ ] вещественное число
- [x] логическое значение
- [ ] целое число
- [ ] строка

![alt text](https://lecontent.sololearn.com/material-images/0d376f3c9c6e4212be095d0528b66d4d-sense1.png)

Выберите названия книг, для которых данное условие верно в таблице выше:
- [x] The Hobbit
- [ ] The Lord of the Rings
- [x] A Game of Thrones
- [ ] Harry Potter
...
**Булево значение** - это тип данных с двумя значениями: `True` или `False` (**Правда или Ложь**)

![alt text](https://lecontent.sololearn.com/material-images/3922db109a9b46449da04262095e8a1c-Frame27166.png)

Логическая операция принимает **несколько булевых значений** и производит **одно логическое значение**.

![alt text](https://lecontent.sololearn.com/material-images/bad7547570f44b829bdc06ed01f408ea-Frame27167.png)

Результатом операции **AND** является значение `True` только тогда, когда **все значения одновременно** `True`. 
...
![alt text](<https://lecontent.sololearn.com/material-images/d7a85ebd50464c1c97fa16a4991a595a-Fra71901).png>)

Вы можете использовать логические операции для фильтрации данных, которые соответствуют нескольким условиям. Операция **AND** дает значение `True` только тогда, **когда все условия `True`**.
```sql
SELECT title
FROM books
...
![alt text](https://lecontent.sololearn.com/material-images/8270fe596a8d483385021f8c77640762-Frame.27142.png)

Выберите название книги, которая соответствует двум условиям
- [x] A Game of Thrones
- [ ] The Lord of the Rings
- [ ] The Hobbit

![alt text](https://lecontent.sololearn.com/material-images/f3aadf1b0f254c9b9c4583e42d806694-Frame27192.png)

Завершите логическую операцию, чтобы получить значение `False` в результате:
```sql
False AND True
```

![alt text](https://lecontent.sololearn.com/material-images/850a1c36566945bf8e0aa1de4e742f5d-4.06.12.png)

Какие значения id соответствуют нескольким условиям в запросе?
- [x] 1
- [ ] 4
- [x] 5

![alt text](https://lecontent.sololearn.com/material-images/fa54720c7e734af384939f80295dd673-4.06.13.png)

Следуюдий запрос демонстрирует как получить `id` заказов, которые были `delivered` в `USA`:
```sql
SELECT id
FROM orders
...
![alt text](https://lecontent.sololearn.com/material-images/c78ac874816f4e49aed9160889f37734-4.06.14.png)
...
![alt text](https://lecontent.sololearn.com/material-images/887e49a38f744ccb8690e0238a8ded4c-4.06.15.png)

```sql
False OR False
``` 

Результатом логической операции выше - будет `False`.

![alt text](https://lecontent.sololearn.com/material-images/3dfede5454654f5eb9cfa17d24993447-4.06.17.png)

Выберите книги, которые будут извлечены из вышеприведенной таблицы:
- [ ] A Game of Thrones
- [x] The Lord of the Rings
- [x] Harry Potter
- [ ] The Hobbit

Следующий запрос демонстрирует как получить продукты из категорий `clothing` или `shoes`:
```sql
SELECT *
FROM products
...
Запрос, чтобы получить пользователя с именем пользователя `ann88` или номером телефона `854930340`:
```sql
SELECT *
FROM users
...
![alt text](https://lecontent.sololearn.com/material-images/3cea210b1a5c46bbb43b933badd3db8e-4.06.20.png)

**Визуализация данных** помогает выявлять тенденции и паттерны в ваших данных. Язык программирования, такой как Python, позволяет создавать мощные визуальные средства для передачи ваших результатов! **Завершен следующий шаг для дополнения ваших новых навыков SQL!**

![alt text](https://lecontent.sololearn.com/material-images/c80613ebcef443f58db957b45d6d3a7a-Frame27180.png)
